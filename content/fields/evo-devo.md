---
id: evo-devo
domain: biology
thread: development
name: Evolutionary Developmental Biology
parent_ids:
  - developmental-genetics
  - evolutionary-biology
era_emerged: 1984 – 2005
core_question: If all animals are built with the same genes, how did evolution make them so different?

summary: |-
  Evolutionary developmental biology, or evo-devo, asks how changes in development produce changes in body form. Evolution can only alter an animal by altering the way it grows from an egg, so the genes that control development are where new shapes must come from.

  Its founding surprise, in the 1980s and 1990s, was that animals as different as flies, worms, fish and people build their bodies with the same small kit of master genes. Genes that lay out the fly's head-to-tail axis do the same job in mice, and a gene needed for the eye in flies is needed for the eye in humans. The differences between animals lie less in which genes they have than in when and where those genes are switched on. Fossils, and the genetics of fish that have evolved in the wild, have since shown particular changes of this kind at work.

key_ideas:
  - term: The genetic toolkit
    definition: >-
      A small set of master genes and signalling pathways, shared by nearly all animals,
      that lays out the body plan. It is at least 550 million years old.
    turning_point_id: evodevo-hox-conservation
  - term: Master control gene
    definition: >-
      A gene that can switch on the whole programme for building an organ. Switching on
      eyeless in the wrong place makes a fly grow extra eyes there.
    turning_point_id: pax6-eyeless
  - term: Deep homology
    definition: >-
      Organs that evolved separately, such as a fly's leg and a mouse's leg, can be built
      by the same ancient genes. The organs are not related, but their genetic machinery is.
    turning_point_id: evodevo-deep-homology
  - term: Regulatory evolution
    definition: >-
      Changes to the switches that control when and where a gene is used, rather than to
      the protein it makes. A switch can change one body part without harming the others
      that depend on the same gene.
    turning_point_id: stickleback-pitx1
  - term: Transitional form
    definition: >-
      A fossil with a mixture of ancestral and descendant features, showing the steps of a
      major change such as fins becoming limbs.
    turning_point_id: evodevo-tiktaalik

turning_points:
  - id: evodevo-hox-conservation
    date: 1984 – 1989
    type: CONSENSUS-OVERTURNED
    title: The same genes in flies and mice
    description: >-
      Soon after the homeobox was found in fly genes, it is found in frogs, mice and
      humans. In 1989 two groups, Denis Duboule's and Robb Krumlauf's, show that mouse Hox
      genes sit in clusters in the same order as the fly's, and are switched on along the
      body in the same order too. Biologists had expected the genes that build a fly and a
      mouse to be unrelated, since their bodies have little in common.
    contested: false
    sources:
      - citation: "McGinnis, W., Garber, R. L., Wirz, J., Kuroiwa, A. & Gehring, W. J. (1984). A homologous protein-coding sequence in Drosophila homeotic genes and its conservation in other metazoans. Cell 37(2): 403–408."
        url: null
      - citation: "Duboule, D. & Dollé, P. (1989). The structural and functional organization of the murine HOX gene family resembles that of Drosophila homeotic genes. EMBO Journal 8(5): 1497–1505."
        url: null
      - citation: "Graham, A., Papalopulu, N. & Krumlauf, R. (1989). The murine and Drosophila homeobox gene complexes have common features of organization and expression. Cell 57(3): 367–378."
        url: null

  - id: pax6-eyeless
    date: 1994 – 1995
    type: DISCOVERY
    title: One gene for eyes
    description: >-
      Walter Gehring's group finds that the fly gene eyeless is the same gene as Pax6,
      which is needed for eyes in mice and humans. In 1995 Georg Halder, Patrick Callaerts
      and Gehring switch on eyeless in the legs, wings and antennae of flies, and extra
      compound eyes grow there. The mouse version of the gene works in flies too, although
      a fly eye and a mouse eye are built completely differently.
    contested: true
    contested_note: >-
      Gehring argued that Pax6 shows all animal eyes share one origin. Others hold that
      eyes evolved independently many times, as comparative anatomy had suggested, and
      that an ancient gene for light-sensing cells was recruited into each separately. What
      Pax6 implies about the history of eyes is still debated.
    sources:
      - citation: "Quiring, R., Walldorf, U., Kloter, U. & Gehring, W. J. (1994). Homology of the eyeless gene of Drosophila to the Small eye gene in mice and Aniridia in humans. Science 265(5173): 785–789."
        url: null
      - citation: "Halder, G., Callaerts, P. & Gehring, W. J. (1995). Induction of ectopic eyes by targeted expression of the eyeless gene in Drosophila. Science 267(5205): 1788–1792."
        url: null

  - id: evodevo-deep-homology
    date: "1997"
    type: SYNTHESIS
    title: Deep homology
    description: >-
      Neil Shubin, Cliff Tabin and Sean Carroll review how limbs evolved and find that the
      appendages of insects, fish and tetrapods, which share no common ancestor with limbs,
      are all patterned by the same genes. They call this deep homology. The genes were
      older than the organs, and were borrowed again and again to build new ones.
    contested: false
    sources:
      - citation: "Shubin, N., Tabin, C. & Carroll, S. (1997). Fossils, genes and the evolution of animal limbs. Nature 388: 639–648."
        url: null
      - citation: "Shubin, N., Tabin, C. & Carroll, S. (2009). Deep homology and the origins of evolutionary novelty. Nature 457: 818–823."
        url: null

  - id: stickleback-pitx1
    date: 2004 – 2010
    type: DISCOVERY
    title: Sticklebacks lose their spines
    description: >-
      Sea-living threespine sticklebacks have a pelvis armed with spines. Many lake
      populations, founded after the last ice age, have lost them. David Kingsley's group
      traces the loss to the gene Pitx1, whose protein is unchanged. What has changed is a
      switch that turns the gene on in the pelvis. In 2010 they show that lake populations
      have deleted that switch independently, again and again.
    contested: false
    sources:
      - citation: "Shapiro, M. D. et al. (2004). Genetic and developmental basis of evolutionary pelvic reduction in threespine sticklebacks. Nature 428: 717–723."
        url: null
      - citation: "Chan, Y. F. et al. (2010). Adaptive evolution of pelvic reduction in sticklebacks by recurrent deletion of a Pitx1 enhancer. Science 327(5963): 302–305."
        url: null

  - id: evodevo-tiktaalik
    date: 2004 – 2006
    type: DISCOVERY
    title: Tiktaalik
    description: >-
      Reasoning that the move from water to land happened in the Late Devonian, Neil
      Shubin, Edward Daeschler and Farish Jenkins search rocks of that age in the Canadian
      Arctic, and in 2004 find a fish about 375 million years old with a flat head, a neck,
      and fins with wrist-like bones. Tiktaalik showed how the bones of the fin became
      those of the limb, and gave developmental genetics a fossil to test itself against.
    contested: false
    sources:
      - citation: "Daeschler, E. B., Shubin, N. H. & Jenkins, F. A. (2006). A Devonian tetrapod-like fish and the evolution of the tetrapod body plan. Nature 440: 757–763."
        url: null
      - citation: "Shubin, N. H., Daeschler, E. B. & Jenkins, F. A. (2006). The pectoral fin of Tiktaalik roseae and the origin of the tetrapod limb. Nature 440: 764–771."
        url: null

  - id: evodevo-cis-regulation
    date: 1975 – 2007
    type: SYNTHESIS
    title: Evolution by genetic switches
    description: >-
      In 1975 Mary-Claire King and Allan Wilson argue that humans and chimpanzees differ
      too little in their proteins to explain their differences, and that changes in gene
      regulation must matter more. Sean Carroll's Endless Forms Most Beautiful (2005) makes
      the case that changes to the switches controlling toolkit genes are the main source
      of new body forms.
    contested: true
    contested_note: >-
      In 2007 Hopi Hoekstra and Jerry Coyne argued that the evidence for the primacy of
      regulatory changes was weaker than claimed, and that changes to proteins explain
      many adaptations. How much of the evolution of form comes from each kind of change
      is still argued.
    sources:
      - citation: "King, M.-C. & Wilson, A. C. (1975). Evolution at two levels in humans and chimpanzees. Science 188(4184): 107–116."
        url: null
      - citation: "Carroll, S. B. (2005). Endless Forms Most Beautiful: The New Science of Evo Devo. W. W. Norton."
        url: null
      - citation: "Hoekstra, H. E. & Coyne, J. A. (2007). The locus of evolution: evo devo and the genetics of adaptation. Evolution 61(5): 995–1016."
        url: null

open_problems:
  - id: evodevo-novelty
    name: How do genuinely new structures evolve?
    status: open
    status_note: Open as of writing; case studies exist, a general account does not.
    description: >-
      Evo-devo explains well how existing parts change shape or are lost. It is less clear
      how wholly new structures arise, such as feathers, the turtle's shell or the
      placenta. Do they always start as a toolkit gene network borrowed from elsewhere,
      and if so, what makes a borrowed network produce something new?
    why_hard: >-
      The first steps happened hundreds of millions of years ago and left few fossils. The
      networks involved contain dozens of genes, and showing that one change started a new
      structure requires comparing living relatives that branched off at just the right
      time.
    unlocks: >-
      A theory of how evolution creates, not only modifies, and of the origin of the major
      animal body plans.
    sources:
      - citation: "Shubin, N., Tabin, C. & Carroll, S. (2009). Deep homology and the origins of evolutionary novelty. Nature 457: 818–823."
        url: null
      - citation: "Wagner, G. P. (2014). Homology, Genes, and Evolutionary Innovation. Princeton University Press."
        url: null

applications:
  - area: Medicine
    title: Hox genes and human hands
    description: >-
      Mutations in HOXD13, one of the Hox genes that pattern the tips of limbs, cause
      synpolydactyly, extra and fused fingers and toes. Knowing how the Hox genes pattern
      limbs in mice and chicks explained why.
    sources:
      - citation: "Muragaki, Y., Mundlos, S., Upton, J. & Olsen, B. R. (1996). Altered growth and branching patterns in synpolydactyly caused by mutations in HOXD13. Science 272(5261): 548–551."
        url: null
  - area: Agriculture
    title: How maize was made
    description: >-
      Maize was domesticated from a bushy wild grass, teosinte. Much of the change comes
      from the gene teosinte branched1, whose switch is set higher in maize, suppressing
      side branches. Crop domestication is evo-devo at speed, driven by human selection.
    sources:
      - citation: "Doebley, J., Stec, A. & Hubbard, L. (1997). The evolution of apical dominance in maize. Nature 386: 485–488."
        url: null

further_reading:
  - citation: "Carroll, S. B. (2005). Endless Forms Most Beautiful: The New Science of Evo Devo. W. W. Norton."
    url: null
    note: The standard popular introduction, by one of the field's founders.
  - citation: "Shubin, N. (2008). Your Inner Fish: A Journey into the 3.5-Billion-Year History of the Human Body. Pantheon."
    url: null
    note: The discovery of Tiktaalik, and what our bodies share with fish.
  - citation: "Gehring, W. J. (1998). Master Control Genes in Development and Evolution: The Homeobox Story. Yale University Press."
    url: null
    note: The homeobox and eyeless stories told by the leader of the laboratory.
---

## An Unexpected Unity

{{fig:darwin|Darwin}}, in an 1860 letter, called embryology "by far the strongest single class of facts" in favour of evolution, and Haeckel built a theory on it. But in the twentieth century [evolutionary biology](/biology/evolutionary-biology/) became a science of gene frequencies in populations, and development dropped out of it. Most biologists assumed that a fly and a mouse, which share no body parts, are built by unrelated genes.

The homeobox overturned that. Within months of its discovery in fly genes by [developmental genetics](/biology/developmental-genetics/), {{fig:william-mcginnis|William McGinnis}} and others found it in frogs, mice and humans. By 1989 the mouse Hox genes were known to sit in clusters, in the same order as the fly's, and to be switched on along the body in the same order, head to tail. The last common ancestor of flies and mice, a small animal living well over 550 million years ago, already used them to tell its front from its back.

## Eyes and Limbs

Eyes seemed the clearest case of separate invention. The compound eye of a fly, with hundreds of facets, and the camera eye of a mouse, with one lens, have nothing in common. Yet {{fig:walter-gehring|Walter Gehring}}'s laboratory found in 1994 that the fly's eyeless gene and the mouse's Pax6 gene are the same gene, needed for eyes in both. In 1995 {{fig:georg-halder|Georg Halder}} and {{fig:patrick-callaerts|Patrick Callaerts}} switched eyeless on in the wrong places and grew fly eyes on legs, wings and antennae. The mouse gene did the same in a fly, making fly eyes, not mouse eyes.

{{fig:neil-shubin|Neil Shubin}}, Cliff Tabin and {{fig:sean-carroll|Sean Carroll}} named the pattern in 1997: deep homology. A fly's leg and a fish's fin are not descended from a common appendage, but the genes that pattern them are. Evolution builds new organs by reusing old instructions. Shubin tested this in the rocks as well. With {{fig:edward-daeschler|Edward Daeschler}} and Farish Jenkins he searched Arctic rocks of the right age for a fish on its way to walking, and in 2004 found Tiktaalik. Its fin held the beginnings of a wrist.

## A Closer Look: Counting Hox Genes

Why do mice have more Hox genes than flies? Count them:

| Animal | Hox clusters | Hox genes |
|---|---|---|
| Fruit fly | 1 (split into two parts) | 8 |
| Amphioxus (lancelet) | 1 | about 15 |
| Mouse or human | 4 | 39 |
| Zebrafish | 7 | about 49 |

Amphioxus is a small, fish-shaped relative of the vertebrates, and its single cluster looks like the ancestral vertebrate one. Early in vertebrate history the whole genome was copied twice, so one cluster became

$$
2^2 = 4
$$

clusters, now called A, B, C and D. In mammals the Hox genes fall into 13 numbered groups, by their position in the cluster. If all four clusters had kept a gene in every group, there would be $4 \times 13 = 52$. Humans have $11 + 10 + 9 + 9 = 39$, so

$$
\frac{39}{52} = 0.75 ,
$$

three quarters of the possible slots are filled, and 13 copies were lost. The ancestors of zebrafish and most other ray-finned fish copied their genome a third time, making $2^3 = 8$ clusters and $8 \times 13 = 104$ possible slots. Zebrafish have seven clusters left and about 49 Hox genes, fewer than half.

Most spare copies are lost. The ones that survive often take on new jobs, and some are later found at the heart of vertebrate novelties. The Hox genes of groups 9 to 13 in the A and D clusters pattern the limbs, and a late phase of Hoxd13 activity builds the fingers and toes. Duplication gives evolution spare copies to experiment with while the original keeps working.

## Switches

If the kit is shared, what makes animals differ? Mostly, it seems, when and where the genes are used. Each toolkit gene has many switches, stretches of nearby DNA that turn it on in particular tissues. Changing one switch alters one body part and leaves the rest alone, while changing the protein would alter every tissue that uses it. The threespine stickleback shows this at work. When sea fish colonised lakes after the last ice age, many populations lost their pelvic spines. {{fig:david-kingsley|David Kingsley}}'s group traced the loss to a deleted switch for the gene Pitx1, which is also needed for the jaw and pituitary. The protein was intact. The same deletion had happened independently in lake after lake.

How much of evolution works this way is argued over. Carroll's 2005 book made the case that switches matter most, and others replied that changes to proteins explain many adaptations too. The field now lies between [developmental genetics](/biology/developmental-genetics/) and [population genetics](/biology/population-genetics/), and the open question is how genuinely new structures, such as feathers or the turtle's shell, first arose.
