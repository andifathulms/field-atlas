---
id: genetics
domain: biology
thread: heredity
name: Genetics
parent_ids: []
era_emerged: 1866 – 1915
core_question: How are traits passed from parents to offspring?

summary: |-
  Genetics is the science of heredity. Traits are passed on through discrete units, genes, that come in pairs, one from each parent, and are shuffled but not blended from one generation to the next. That is why a trait can skip a generation and reappear, and why children are neither averages of their parents nor copies of them.

  Mendel found the rules in the 1860s by counting pea plants, and no one noticed for thirty-four years. Once rediscovered, genetics located genes on chromosomes, learned to create mutations, and connected genes to the chemistry of cells, setting the stage for molecular biology.

key_ideas:
  - term: Gene and allele
    definition: >-
      A gene is a unit of heredity. Alleles are its alternative versions, such as the
      versions for purple or white flowers. Most plants and animals carry two alleles of each
      gene, one from each parent.
    turning_point_id: mendel-peas
  - term: Dominant and recessive
    definition: >-
      When the two alleles differ, one (dominant) may mask the other (recessive), which
      reappears only when an offspring inherits two recessive copies. That gives the
      classic 3 : 1 ratio.
    turning_point_id: mendel-peas
  - term: Segregation and independent assortment
    definition: >-
      The two alleles of a gene separate into different sex cells, and different genes
      are, as a first approximation, inherited independently of each other.
    turning_point_id: mendel-peas
  - term: Chromosomes and linkage
    definition: >-
      Genes sit in a line along chromosomes. Genes close together tend to be inherited
      together, and how often they are separated measures their distance.
    turning_point_id: chromosome-theory
  - term: Genotype and phenotype
    definition: >-
      The genotype is the set of alleles an organism carries. The phenotype is what it
      looks like and how it works, which depends on both genes and environment.

turning_points:
  - id: mendel-peas
    date: "1866"
    type: DISCOVERY
    title: Mendel's experiments on plant hybrids
    description: >-
      Gregor Mendel, an Augustinian friar in Brno, grows tens of thousands of pea plants
      over eight years and counts their traits. The ratios, 3 : 1 in the second generation,
      show that heredity works through discrete factors that segregate and recombine
      without blending. The paper appeared in a local natural-history journal and was
      cited only a handful of times in thirty-four years.
    contested: true
    contested_note: >-
      In 1936 the statistician Ronald Fisher argued that Mendel's data fit his
      expectations too closely to be plausible, suggesting they had been adjusted,
      perhaps by an assistant. The charge has been debated ever since. Later analyses
      have proposed innocent explanations, such as how plants were selected or counted,
      without settling the question. That Mendel's conclusions were correct is not
      disputed.
    sources:
      - citation: "Mendel, G. (1866). Versuche über Pflanzen-Hybriden. Verhandlungen des naturforschenden Vereines in Brünn 4: 3–47."
        url: null
      - citation: "Fisher, R. A. (1936). Has Mendel's work been rediscovered? Annals of Science 1(2): 115–137."
        url: https://doi.org/10.1080/00033793600200111
      - citation: "Franklin, A., Edwards, A. W. F., Fairbanks, D. J., Hartl, D. L. & Seidenfeld, T. (2008). Ending the Mendel–Fisher Controversy. University of Pittsburgh Press."
        url: null

  - id: mendel-rediscovery
    date: "1900"
    type: DISCOVERY
    title: Mendel's laws are rediscovered
    description: >-
      Within a few months, Hugo de Vries, Carl Correns and Erich von Tschermak each
      publish breeding results that match Mendel's, and each cites his forgotten paper.
      William Bateson champions the new science and in 1905 names it *genetics*.
    contested: true
    contested_note: >-
      How independent the three rediscoveries were is debated. De Vries's first
      publication did not mention Mendel, and historians have questioned how far
      Tschermak understood the laws he was confirming. The "rediscovery" is sometimes
      described as a convergence in which Mendel's paper was found after the fact and
      used to settle priority among the three.
    sources:
      - citation: "Olby, R. C. (1985). Origins of Mendelism (2nd ed.). University of Chicago Press."
        url: null

  - id: chromosome-theory
    date: 1902 – 1915
    type: SYNTHESIS
    title: Genes are located on chromosomes
    description: >-
      Walter Sutton and Theodor Boveri notice that chromosomes behave during cell
      division exactly as Mendel's factors should. From 1910 Thomas Hunt Morgan's "fly
      room" at Columbia follows a white-eyed mutant fruit fly and ties the trait to a
      sex chromosome. In 1913 his student Alfred Sturtevant draws the first genetic map.
      Heredity and cell biology become one theory.
    contested: false
    sources:
      - citation: "Morgan, T. H. (1910). Sex limited inheritance in Drosophila. Science 32(812): 120–122."
        url: https://doi.org/10.1126/science.32.812.120
      - citation: "Sturtevant, A. H. (1913). The linear arrangement of six sex-linked factors in Drosophila, as shown by their mode of association. Journal of Experimental Zoology 14(1): 43–59."
        url: https://doi.org/10.1002/jez.1400140104

  - id: muller-xrays
    date: "1927"
    type: TECHNIQUE-INVENTED
    title: X-rays create mutations on demand
    description: >-
      Hermann Muller shows that X-rays greatly increase the rate of mutation in fruit
      flies. Geneticists no longer have to wait for rare natural mutants, and the
      finding carries a warning, repeated by Muller for the rest of his life, about the
      genetic dangers of radiation.
    contested: false
    sources:
      - citation: "Muller, H. J. (1927). Artificial transmutation of the gene. Science 66(1699): 84–87."
        url: https://doi.org/10.1126/science.66.1699.84

  - id: one-gene-one-enzyme
    date: "1941"
    type: DISCOVERY
    title: One gene, one enzyme
    description: >-
      George Beadle and Edward Tatum irradiate bread mould and find mutants that each
      cannot make one particular nutrient because a single enzyme is broken. Genes work
      by specifying the proteins that run the cell's chemistry. The question "what is a
      gene made of?" becomes urgent, and molecular biology begins.
    contested: false
    sources:
      - citation: "Beadle, G. W. & Tatum, E. L. (1941). Genetic control of biochemical reactions in Neurospora. Proceedings of the National Academy of Sciences 27(11): 499–506."
        url: https://doi.org/10.1073/pnas.27.11.499

open_problems:
  - id: transgenerational-epigenetics
    name: Can acquired traits be inherited?
    status: conjectured
    status_note: Well documented in plants and worms; contested in mammals, including humans, as of writing.
    description: >-
      Genes can be switched on and off by chemical marks that are not changes in the DNA
      sequence. In some organisms such epigenetic marks, influenced by diet or stress,
      pass to later generations. Whether this happens meaningfully in mammals would
      revive, in a limited form, the old Lamarckian idea that acquired traits can be
      inherited.
    why_hard: >-
      Most epigenetic marks in mammals are erased twice, in the early embryo and in the
      germ cells. Effects seen across generations can also be carried by the uterine
      environment, behaviour or the microbiome rather than by the marks themselves.
      Disentangling these requires long, carefully controlled multi-generation studies.
    unlocks: >-
      It would change how the effects of famine, toxins and stress on future
      generations are understood, and it would add a second channel of inheritance
      alongside DNA.
    sources:
      - citation: "Heard, E. & Martienssen, R. A. (2014). Transgenerational epigenetic inheritance: myths and mechanisms. Cell 157(1): 95–109."
        url: https://doi.org/10.1016/j.cell.2014.02.045

applications:
  - area: Agriculture
    title: Scientific plant and animal breeding
    description: >-
      Once inheritance followed rules, breeding could be planned rather than guessed.
      Hybrid maize in the 1930s and the high-yield semi-dwarf wheats and rices of the
      1960s Green Revolution came out of Mendelian breeding programmes, and they fed
      hundreds of millions of people.
    sources: []
  - area: Medicine
    title: Newborn screening for inherited disease
    description: >-
      Phenylketonuria, a recessive disorder that causes severe intellectual disability
      if untreated, can be managed by diet if caught early. Robert Guthrie's 1963 blood
      test made it possible to screen every newborn. Many countries now screen for
      dozens of inherited conditions at birth.
    sources:
      - citation: "Guthrie, R. & Susi, A. (1963). A simple phenylalanine method for detecting phenylketonuria in large populations of newborn infants. Pediatrics 32(3): 338–343."
        url: https://doi.org/10.1542/peds.32.3.338

further_reading:
  - citation: "Mukherjee, S. (2016). The Gene: An Intimate History. Scribner."
    url: null
    note: A sweeping popular history from Mendel to gene editing, braided with family memoir.
  - citation: "Henig, R. M. (2000). The Monk in the Garden: The Lost and Found Genius of Gregor Mendel. Houghton Mifflin."
    url: null
    note: A narrative biography of Mendel and his rediscovery.
  - citation: "Griffiths, A. J. F., Wessler, S. R., Carroll, S. B. & Doebley, J. (2015). Introduction to Genetic Analysis (11th ed.). W. H. Freeman."
    url: null
    note: A standard university textbook.
---

## Counting Peas

Before genetics, heredity was thought to blend. A child was a mixture of its parents, as paint mixes. That explained why offspring resemble both parents, but not why traits skip generations, and, as Darwin's critics pointed out, it would dilute any new variation to nothing.

{{fig:mendel|Gregor Mendel}}, a friar and teacher at the Augustinian monastery in Brno, took a different approach: choose simple traits, breed carefully, and count. Crossing purple-flowered with white-flowered peas gave all purple offspring. Crossing those with each other gave purple and white again, in a ratio of about three to one. The white trait had not blended away. It had been hidden. Mendel explained it with paired factors, one from each parent, that separate cleanly when sex cells form. He presented the work in 1865 and published it in 1866. For thirty-four years almost no one took notice.

## Rediscovery and the Fly Room

In 1900 three botanists, {{fig:de-vries|Hugo de Vries}}, {{fig:correns|Carl Correns}} and Erich von Tschermak, reported similar results and found Mendel's paper. William Bateson made himself its champion and gave the new science its name. Wilhelm Johannsen coined *gene* in 1909.

Where were the genes? Chromosomes, the thread-like bodies that pair up and separate when cells divide, behaved exactly as Mendel's factors should. In 1910 {{fig:morgan|Thomas Hunt Morgan}} found a single white-eyed male fruit fly in his crowded Columbia lab and traced the trait to the X chromosome. His student {{fig:sturtevant|Alfred Sturtevant}}, then an undergraduate, realised overnight that how often linked genes are separated measures their distance, and drew the first genetic map.

## A Closer Look: Mendel's Counts

Mendel's key experiment crossed pure-breeding round-seeded peas with pure-breeding wrinkled ones. All the offspring were round. He then let those hybrids self-fertilise and counted their seeds:

| Trait | Dominant | Recessive | Ratio |
|---|---|---|---|
| Seed shape | 5,474 round | 1,850 wrinkled | 2.96 : 1 |
| Seed colour | 6,022 yellow | 2,001 green | 3.01 : 1 |

Why three to one? Suppose each plant carries two copies of a factor, now called a gene, one from each parent. Write $R$ for round, which is dominant, and $r$ for wrinkled. The hybrids are all $Rr$. Each passes on $R$ or $r$ with equal chance, so their offspring are

$$
\tfrac14\, RR + \tfrac12\, Rr + \tfrac14\, rr .
$$

Anything with at least one $R$ looks round, so three-quarters are round and a quarter wrinkled. The wrinkled factor was hidden in the hybrids, not lost.

Mendel then tracked two traits at once. If the genes for shape and colour are passed on independently, the combinations should appear in the proportions $\tfrac34 \times \tfrac34$, $\tfrac34 \times \tfrac14$, $\tfrac14 \times \tfrac34$ and $\tfrac14 \times \tfrac14$, a ratio of 9 : 3 : 3 : 1. Of 556 seeds, the expected numbers are 312.75, 104.25, 104.25 and 34.75. He counted 315 round yellow, 101 wrinkled yellow, 108 round green and 32 wrinkled green.

The fit is close, perhaps too close. In 1936 Ronald Fisher calculated that Mendel's data agree with the theory better than chance would usually allow, and suggested that someone, perhaps an assistant, had tidied them. Others have argued that Mendel's methods, such as when he stopped counting, explain the fit. Either way, the ratios have been confirmed countless times since. Independence also has limits: genes close together on the same chromosome tend to travel together, which is how Sturtevant built the first genetic map.

## From Heredity to Chemistry

Genetics then learned to intervene. In 1927 {{fig:muller|Hermann Muller}} found that X-rays multiply the mutation rate, giving geneticists an endless supply of mutants and giving the world an early warning about radiation. In 1941 {{fig:beadle|George Beadle}} and {{fig:tatum|Edward Tatum}} used that trick on bread mould and found that each mutation disabled one enzyme. Genes work by making proteins.

That turned heredity into a question about molecules. What is a gene made of, and how does it make a protein? Answering it created [molecular biology](/biology/molecular-biology/). Meanwhile Mendel's discrete genes rescued Darwin. Because they do not blend, variation is preserved, and the mathematical fusion of the two founded [population genetics](/biology/population-genetics/).
