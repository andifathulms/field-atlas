...
**Second and third threads in physics (done):** the Entropy Thread, five fields. Thermodynamics is the root. Kinetic Theory rests on it and on Classical Mechanics, Statistical Mechanics follows, and Phase Transitions and Non-Equilibrium Physics branch from it. The Quantum Thread, five fields. Old Quantum Theory is born at the seam of Statistical Mechanics and Electromagnetism, and Quantum Mechanics follows. Quantum Field Theory (with Special Relativity) and Quantum Information branch from it, and Particle Physics grows from Quantum Field Theory. Nine maths applications that had pointed at physics without a surveyed field now land in these fields. The new physics fields have closer-look chapters.

**Fourth physics thread and second biology thread (done):** the Stars Thread, five fields. Astronomical Spectroscopy draws on Electromagnetism and Old Quantum Theory. Stellar Astrophysics adds Quantum Mechanics, Compact Objects adds General Relativity, Galactic Astronomy branches from Spectroscopy, and Exoplanetary Science joins Spectroscopy and Stellar Astrophysics. The Cell Thread, five fields. Cell Theory is the root. Microbiology and Biochemistry branch from it, Cell Biology joins Cell Theory and Biochemistry, and Immunology joins Microbiology and Cell Biology. Biochemistry is also now a second parent of Molecular Biology, across threads. Every new field has a closer-look chapter, and the original Relativity and Heredity fields have since gained one, so every field in the atlas now has a worked example.

**Seven more threads (done):** biology gained the Brain Thread (Neuroanatomy from Cell Theory; Electrophysiology; Synaptic Transmission with Biochemistry; Systems and Computational Neuroscience), the Ecology Thread (Biogeography from Evolutionary Biology; Population, Community and Ecosystem Ecology; Conservation Biology with Population Genetics) and the Development Thread (Embryology as a root; Experimental Embryology; Developmental Genetics with Genetics; Evo-Devo with Evolutionary Biology; Stem Cell Biology with Cell Biology). Physics gained the Matter Thread (Crystallography from Electromagnetism; Solid-State Physics with Quantum Mechanics; Superconductivity and Topological Matter with Phase Transitions; Magnetism with Statistical Mechanics) and the Nuclear Thread (Radioactivity from Electromagnetism; Nuclear Structure with Quantum Mechanics; Nuclear Fission; Fusion Energy with Stellar Astrophysics; Radiometric Dating). Mathematics gained the Statistics Thread (Statistical Inference, Bayesian Statistics, Information Theory and Stochastic Processes from Probability Theory; Statistical Learning Theory with Computational Complexity) and the Computation Thread (Numerical Analysis from Calculus; Numerical Linear Algebra; Numerical PDEs with Differential Equations; Monte Carlo Methods with Probability Theory; Continuous Optimisation with Real Analysis). The atlas now surveys 104 fields in 20 threads.

**Consolidation (done):** every field was fact-checked against sources, with corrections committed thread by thread and open-problem statuses dated to 2026 (for example, Hilbert's sixth problem and the McKay conjecture are now recorded as resolved). Navigation was added for the atlas's size: a people index, site search, an all-threads map on the landing page, an open problems page, and the long-planned toolkit view, which ranks fields by how far their ideas travelled across threads and domains.
