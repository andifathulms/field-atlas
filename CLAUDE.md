...
- `content/fields/<field-id>.md`: YAML frontmatter holds the structured fields (id, domain, thread, name, parent_ids, era_emerged, core_question, turning_points, open_problems). The Markdown body is split into chapters on `## ` headings, so chapter count and titles are free per field.
...
`src/lib/treeLayout.ts` is a manual layered layout. Longest-path layers are computed first; any row holding more than three fields sends its latest-born field (by `era_emerged`) down a row. Every left-to-right ordering of each layer is then placed at parent barycenters and scored: branches through another field's dot or trail cost the most, then branches through its labels, then branch crossings (checked cheaply because every branch is monotone in height), then small terms for distance from the barycenter and sideways travel. The best few orderings are refined by sliding nodes sideways while the score improves. Each node's label width is estimated from its text (`tickLabel`/`eraLabel` are shared with the renderer). The search is exhaustive only while the number of layer orderings stays under `MAX_CANDIDATES`; a much larger tree would need a real crossing-minimisation heuristic. `src/components/FieldTree.tsx` draws the result as SVG, using `d3-shape` for the branch curves. Branches draw in with a `pathLength` dash animation. Unresolved open problems render as fading dashes and fog. Lineage that crosses into another thread's map is drawn as rows inside the node's own label column (`↖ from …` under the era line, `↘ into …` after the waypoints, labels from `src/lib/threadLinks.ts`), because the layout already keeps that column clear of branches. Stubs in the gap between rows were tried and collided with converging branches. Straight lines can't take an `objectBoundingBox` gradient stroke, which is why the fading is done with individual dashes.

## Chrome and reading aids — as built
- Theme: day tokens on `:root`, night tokens under `[data-theme="dark"]` and `prefers-color-scheme: dark` (both blocks in `globals.css` must stay in step). `src/lib/theme.ts` holds the pre-paint script that applies a stored choice; `ThemeToggle` writes it. Never hard-code a color in a component or SVG: use the CSS variables so both themes work.
- The header is sticky. `SiteNav` publishes its height as `--header-h`, which `html { scroll-padding-top }` and the domain page's `ThreadNav` strip both use.
